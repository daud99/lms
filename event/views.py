    ...
    if request.method == "POST":
        ...
        try:
            ...
            if "category_image" in request.FILES:
                category_image = request.FILES["category_image"]
                common.uploadImageToFTP(category_image.name, category_image)
                EventCategories.objects.filter(id=id).update(category_name=category_name, category_code=category_code, category_image_url=category_image.name, category_priority=category_priority, status=category_status, updated_user=current_user, updated_date=int(time.time()))
            else:
